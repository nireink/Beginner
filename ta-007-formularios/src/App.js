...
function App() {
  ...
  function cambiarRubro(e) {
    const codigoRubro = Number(e.target.value); // <-- convertir a número
  ...
  function cambiarArticulo(e) {
    const codigoArticulo = Number(e.target.value); // <-- convertir a número
    setArticulo(articulosRubro.find(a => a.codigo === codigoArticulo) ?? null);
  }
